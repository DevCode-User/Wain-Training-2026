...
import com.example.ec.dto.RegisterResult;
...
public class AuthService {
    ...
    public LoginResult loginCustomer(String loginId, String rawPassword) {
        ...
        if (found.isEmpty()) {
            return new LoginResult(false, "CUSTOMER", null, null);
        }
    ...
    public LoginResult loginAdmin(String loginId, String rawPassword) {
        ...
        if (found.isEmpty()) {
            return new LoginResult(false, "ADMIN", null, null);
        }
    ...
    public RegisterResult registerCustomer(RegisterForm form) {
        if (customerRepository.existsByLoginId(form.getLoginId())) {
            return new RegisterResult(false, "ログインIDはすでに使用されています。", null);
        }
        if (customerRepository.existsByCustomerEmail(form.getCustomerEmail())) {
            return new RegisterResult(false, "メールアドレスはすでに登録されています。", null);
        }
